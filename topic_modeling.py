...
def termFrequency(comments, labels, k, output_dir=None, top_n=20, save=False, translation=None):
    """Return term frequencies"""
    number_of_cluster = int(k)
    # df = zip(comments, labels)
    # df = pd.DataFrame(df)
    # df.columns = ['comments', 'cluster']
    ...
    top_n_words = extract_top_n_words_per_topic(tf_idf, count, docs_per_topic, n=top_n)
    topic_sizes = extract_topic_sizes(docs_df)
    # topic_sizes.head(10)

    def write_topics(topics, distribution, f):
        f.write('Topic distribution: [')
        for key, value in topics.items():
            f.write(f"{distribution[distribution.Topic == key].Size.values[0]}, ")
        f.write(']\n')
        for key, value in topics.items():
            if translation:
                f.write(f"{translation[key]}({key}): {distribution[distribution.Topic == key].Size.values[0]}\n{'-'*20}\n")
            else:
                f.write(f"Topic {key}: {distribution[distribution.Topic == key].Size.values[0]}\n{'-'*20}\n")
            if len(value) == 20:
                idx = 0
                for i in range(4):
                    for j in range(5):
                        to_print = f"{value[idx][0]} [{value[idx][1]:.5f}]"
                        idx += 1
                        f.write(f'{to_print:<15}')
                    f.write('\n')
            f.write('\n')

    if save:
        with open(output_dir + '/term_frequency.txt', 'w', encoding='utf-8') as f:
            write_topics(top_n_words, topic_sizes, f)
